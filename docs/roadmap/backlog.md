...
- **plataforma Git local + CI Runner + registry**: Forgejo 16.0.4 desplegado en `devops01` ([ADR-010](../arquitectura/decisiones-arquitectonicas.md#adr-010--forgejo-con-forgejo-actions-como-plataforma-git-local)), admin creado, `http://git.oscar.home` sin puerto vía [Nginx Proxy Manager](../servicios/nginx-proxy-manager.md#proxy-hosts-reales), CI Runner (`forgejo-runner` 13.1.0) registrado y activo (falta validar con un workflow real), y [Nexus 3.96.1](../servicios/nexus.md) con npm+Docker registry configurados — el pipeline `lint → test → build → push Nexus → GitOps → Argo CD` ya tiene todas sus piezas desplegadas, falta encadenarlas en un workflow real.
