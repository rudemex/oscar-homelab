...
Además del `siteMonitor` (puntito de estado), **8 de las 13 tarjetas** tienen un `widget:` que muestra algo en vivo directo en la card en vez de un link plano. Siete son "bloques de números" — Proxmox, AdGuard Home, Cloudflare Tunnel, Uptime Kuma, Beszel, Home Assistant y [MySpeed](./myspeed.md) — y Glances es la única realmente distinta (ver el porqué más abajo). n8n, Vaultwarden y [DVR Dahua](../hogar/cctv-dahua.md) se quedan con descripción fija: los dos primeros porque Homepage no tiene una integración nativa para ninguno, y el DVR porque se decidió no invertir más tiempo/recursos en tener su video dentro del dashboard (ver más abajo). [Nginx Proxy Manager](./nginx-proxy-manager.md) ya tiene su widget nativo sumado (tipo `npm`, campos `["enabled", "disabled", "total"]`) — quedó documentado acá como "pendiente del login" bastante después de que el login ya se había cambiado, mismo patrón de doc-desactualizada-no-infra-real que ya pasó un par de veces en este proyecto.

**Nota de una auditoría completa de tarjetas** (revisar todas contra sus APIs reales, no solo la config): encontró Proxmox mostrando `NaN` por un token sin permisos (la afirmación de que el rol ya estaba asignado, más abajo en esta página, era falsa), el ícono de Nexus apuntando a un nombre que no existe en `dashboard-icons`, Beszel mostrando una sola VM cuando ya había 3 con agente, y al menos tres descripciones desactualizadas (Forgejo seguía diciendo "mirror" después de migrar a origen real, AdGuard afirmaba ser DNS de toda la red cuando no lo es). Todo corregido — el patrón real: una tarjeta que "se ve bien" (renderiza, no tira error) no significa que los datos sean correctos ni que la descripción siga siendo cierta.
...
### DVR Dahua: de video en vivo a solo estado

Esta tarjeta pasó por varias versiones — un `<img>` construido a mano con fotos refrescándose, el widget nativo `mjpeg` de Homepage (rompía por mixed content, y después por un límite real de Cloudflare con streams de longitud indefinida), [go2rtc](./go2rtc.md) hablando RTSP con el DVR (video real, HD, pero con la orientación de 3 de las 4 cámaras sin resolver — "modo pasillo", el DVR las rota al mostrarlas y el RTSP crudo no trae esa rotación). Después de esa vuelta completa, la decisión fue **no seguir invirtiendo tiempo/recursos en tener video de las cámaras dentro de Homepage** — [go2rtc](./go2rtc.md) se retiró (igual que antes [DVR Proxy](./dvr-proxy.md)), y la tarjeta volvió a ser un link simple con chequeo de estado, como Proxmenux Monitor o n8n:
...
El token `root@pam!homepage` se creó con `privsep=1` (sin permisos hasta asignarle un rol explícito). Esta página decía que el rol `PVEAuditor` ya estaba asignado a mano desde la UI — no era cierto: la tarjeta de Proxmox mostraba `NaN` porque `/access/permissions` devolvía vacío (`403 Permission check failed` en cualquier endpoint real). Se resolvió por SSH al hipervisor con `pveum acl modify / -token "root@pam!homepage" -role PVEAuditor` — la asignación por CLI (`pveum`) no tiene la misma restricción que la API REST de Proxmox.
...
Como Homepage no le da una clase propia al `<button>` del header, hubo que engancharse con [`:has()`](https://developer.mozilla.org/en-US/docs/Web/CSS/:has) (selector CSS relativamente nuevo, pero ya soportado en todos los navegadores evergreen):

```css
#services {
  gap: 2.25rem;   /* separación entre grupos */
}

.service-group-name {
  flex: 1;   /* ocupa el ancho disponible del botón (salvo lo que usa la flecha) */
}
...
button:has(> .service-group-name) {
  ...
  margin-bottom: 0.3rem;   /* poco aire entre el título y sus propias tarjetas */
  border-bottom: 1px solid;   /* la línea divisoria vive acá, no en el <h2> — llega de punta a punta */
}
.service-group-name ~ svg {
  opacity: 0.75 !important;   /* la flecha, siempre visible */
}
```
