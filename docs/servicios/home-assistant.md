...
**Estado:** Actual · Hogar — **VM 106 (`haos-18.3`)** en `oscar-core`, rehecha desde cero el 2026-09-21 (la VM 101 `haos-18.2` se perdió con el SSD SATA de `Backups`, ver [incidente](../arquitectura/estado-actual.md)). **Onboarding hecho (2026-09-23)** — cuenta de admin creada por el usuario. Sirve por `http://192.168.0.198:80` (no `:8123` como se pensó originalmente — coincide con el ingress ya configurado del túnel de Cloudflare, que siempre apuntó a `:80`). **IP todavía provisoria**: se reutilizó la MAC de la VM vieja (`02:DC:F8:B9:49:3F`) pero el router asignó `.198` y no la `.195` de antes; Homepage, el hostname `ha.oscarlab.com.ar` del túnel de Cloudflare y el monitor de Kuma siguen apuntando a `.195`. Pendiente: fijar `192.168.0.195` en Ajustes → Sistema → Red de Home Assistant, o reservar la IP en el router  
