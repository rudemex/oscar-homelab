...
- Suscripción por `Application`, no global: anotaciones `notifications.argoproj.io/subscribe.<trigger>.argocd-led-relay` en el `application.yaml` de cada app que se quiera reflejar. Suscriptas hoy: `ci-demo` (el piloto) y `oscar-led-controller` (2026-09-25, la app que maneja la propia tira: el pod viejo recibe `/deploying`, el rollout `Recreate` lo reemplaza y el nuevo arranca en el estado persistido hasta que llega `/success` → `healthy`). `root-app` es de sync manual, así que un cambio de anotaciones en un `application.yaml` no se aplica hasta disparar ese sync. Las apps de `infra/` no se suscribieron a propósito: se sincronizan solas seguido y llenarían la tira de ruido.

**Por qué hace falta un relay y no alcanza con la Notification en sí:** Argo CD Notifications puede llamar a un webhook, pero no sabe secuenciar "mostrar `success` un rato y después asentarse en `healthy`" — y el propio estado `success` de `oscar-led-controller` no sirve para eso solo: es `temporary` (ver `states.ts`), así que al vencer su ventana de 4s vuelve sola al estado que estaba activo **antes** de `success`, que acá sería `deploying`, no `healthy`. El relay hace las dos llamadas HTTP en el orden correcto (`POST /state/success`, espera 3,5 s, `POST /state/healthy`) para que el resultado final sea el que corresponde.
...
**Prueba con un despliegue de punta a punta (2026-09-25, pedida por el usuario para ver la luz):** un commit trivial a `ci-demo` → CI de Forgejo (~50 s) → tag nuevo en `oscar-gitops` → Argo CD. La tira mostró `deploying` (20:29:16), `success` (20:29:31, cuando el rollout quedó `Healthy`) y `healthy` (20:29:35); los logs del relay muestran exactamente `/deploying`, `/success` y el `healthy` asentado, todos desde la IP del pod real de `argocd-notifications-controller`. Un detalle a notar: `deploying` dura lo que dura el rollout (~13 s acá), porque el trigger `led-success` espera a `Healthy`.

**Hallazgo de esa prueba: parpadeo de `deploying` entre `success` y `healthy`.** Con la espera original de 4,5 s, el `success` de `oscar-led-controller` (que es `temporary`, 4 s) alcanzaba a volver solo a `deploying` medio segundo antes del `healthy` del relay: ~1 s de cometa de más. Corregido bajando la espera a 3,5 s — el `healthy` explícito cancela el fallback antes de que dispare.
