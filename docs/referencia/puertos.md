...
| Nexus | 8081 UI/API, **8082 registry Docker** (puerto HTTP dedicado, nunca por el proxy) | interno (`devops01`) |
...
| Forgejo | 3000 HTTP / 2222 SSH → 22 interno | interno (`devops01`), detrás de NPM en `git.oscar.home` |
| Beszel | 8090 hub / 45876 agente (protocolo propio, no HTTP — nunca usarlo como `siteMonitor`) | interno, un hub + tres agentes (`core01`/`devops01`/`k3s01`) |
| k3s / Traefik | 80/443 (`LoadBalancer` de k3s, IP del nodo `k3s01`) | interno, ingress de todas las apps `*.oscar.home` en k3s |
