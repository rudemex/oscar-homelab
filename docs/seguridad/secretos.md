...
**Primer caso real migrado (2026-09-19):** el `imagePullSecret` `nexus-pull` de `ci-demo` — antes un `Secret` `kubernetes.io/dockerconfigjson` creado a mano, ahora un `InfisicalSecret` (`apps/ci-demo/templates/secrets.yaml`) que arma el `.dockerconfigjson` con un template de Go (`dict`/`b64enc`, sintaxis Sprig) a partir de `NEXUS_USER`/`NEXUS_PASSWORD` guardados en Infisical (`/ci-demo`). Desde 2026-09-20 el mapeo se declara en `values.yaml` (`backend.secrets:` `CLAVE_K8S: NOMBRE_EN_INFISICAL`, `backend.configmap:` para variables no secretas) — ver [Argo CD](../servicios/argocd.md#estructura-del-repo-2026-09-20). Validado: el contenido del Secret resultante es byte-a-byte igual al que existía a mano, y los pods de `ci-demo` siguen sanos sin ningún cambio en `deployment.yaml` (sigue referenciando `nexus-pull` por nombre, nada más cambió).
