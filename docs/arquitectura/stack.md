...
| Orquestación | [k3s](../kubernetes/instalacion-k3s.md) | **Actual** — `k3s01`, un solo nodo | Kubernetes real con bajo overhead en un nodo chico |
| GitOps | [Argo CD](../kubernetes/instalacion-argocd.md) | **Actual** — en `k3s01`, sincroniza `oscar-gitops` desde Forgejo | reconciliación declarativa, UI para aprender el modelo |
| Registry/artefactos | [Nexus Repository](../servicios/nexus.md) | **Actual** — `devops01`, usado por el CI real | un solo servicio para imágenes OCI + proxy npm |
| Git | [Forgejo](../servicios/forgejo.md) | **Actual** (ADR-010) | self-hosted, liviano, corriendo en `devops01`; CI con Forgejo Actions ya desplegado y validado end-to-end |
