...
| n8n + PostgreSQL 17 | Docker en `automation` (migrado de `core01` el 2026-09-23) | Actual | `/srv/oscar/apps/n8n/`, secretos (`POSTGRES_PASSWORD`, `N8N_ENCRYPTION_KEY`) generados únicos con `openssl rand`, no reutilizados. Migración: contenedores parados en `core01`, volúmenes Docker empaquetados con checksum SHA-256 verificado en origen/Mac/destino, recreados en `automation`. Homepage, el ingress rule de Cloudflare (`n8n.oscarlab.com.ar`) y el monitor de Kuma repuntados a `192.168.0.153`. Contenedores viejos en `core01` quedan detenidos (no borrados) como respaldo. Ver [n8n](../automatizacion/n8n-arquitectura.md). |
