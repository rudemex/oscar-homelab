...
| AdGuard Home | LXC (vmid 100) | Actual | **Ya es el DNS de toda la LAN** (2026-09-18) — router (TP-Link Archer, DHCP) reparte `192.168.0.93` primario + `1.1.1.1` secundario a cualquier dispositivo que use DNS automático. 2 vCPU / 1 GB, IP estática, `ratelimit: 300`. Validado con tráfico real (no solo prueba sintética): +397 paquetes UDP durante una sesión de navegación normal, CPU/RAM sin moverse de casi cero, bloqueo real de un dominio de tracking confirmado. Ver [DNS con AdGuard Home](../red/dns-adguard.md). |
