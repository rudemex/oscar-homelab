...
| SearXNG | k3s en `k3s01` (namespace `oscar-ai`) | Actual | Metabuscador, backend de `web_search` para la futura capa OSCAR AI (Fase 1 del spec de Hermes). Chart propio `oscar-gitops/apps/searxng/`, `SEARXNG_SECRET` desde Infisical (`/searxng`), API JSON habilitada. `searxng.oscar.home` solo para probar; el consumo real irá por el Service interno. ~95 MiB. Open WebUI y Hermes todavía no. |
