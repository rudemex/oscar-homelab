...
| `k3s01` — Ubuntu 24.04 LTS + k3s | VM (vmid 103) | Actual | IP estática `192.168.0.150/24`, mismo patrón de creación que `core01`/`devops01`. **CPU virtual cambiada de `kvm64` a `host` (2026-09-20)** — con `kvm64` faltaban SSE4.2/AVX/AVX2 y los binarios nativos modernos (Claude Code) quedaban colgados al 100% de CPU sin salida; requirió reiniciar la VM. Un solo nodo (server+agent), sin HA todavía — eso sigue siendo Objetivo. Corre k3s (con Traefik como ingress controller, viene por defecto) y Argo CD. Ver [Kubernetes y GitOps](../kubernetes/vision-general.md). |
| Argo CD | k3s en `k3s01` | Actual | GitOps real: lee `oscar-gitops` desde Forgejo (`git.oscar.home`, no GitHub — ver la fila de `oscar-gitops` más abajo). Patrón app-of-apps (`root-app`, sync manual por diseño) con 7 Applications debajo, todas con sync automático y self-heal: apps (`oscar-led-controller`, `ci-demo`, `searxng`) e infra (`infisical-operator`, `headlamp`, `argocd-config`, `coredns-custom`). Detalle en [Argo CD](../servicios/argocd.md). Publicado en `argocd.oscar.home` (Traefik, no NPM). |
...
| n8n + PostgreSQL 17 + Redis 7 (modo *queue*) | Docker en `automation` (migrado de `core01` el 2026-09-23) | Actual | `/srv/oscar/apps/n8n/`, secretos (`POSTGRES_PASSWORD`, `N8N_ENCRYPTION_KEY`) generados únicos con `openssl rand`, no reutilizados. Migración: contenedores parados en `core01`, volúmenes Docker empaquetados con checksum SHA-256 verificado en origen/Mac/destino, recreados en `automation`. Homepage, el ingress rule de Cloudflare (`n8n.oscarlab.com.ar`) y el monitor de Kuma repuntados a `192.168.0.153`. Contenedores viejos en `core01` quedan detenidos (no borrados) como respaldo. **Modo *queue* agregado el mismo día**: se sumaron `redis` y `n8n-worker` (concurrency 10) al compose — separa recepción de webhooks/UI (`main`) de la ejecución real de workflows. Ver [n8n](../automatizacion/n8n-arquitectura.md). |
| Vaultwarden 1.37.2 | Docker en `services` (migrado de `core01` el 2026-09-23) | Actual | `/srv/oscar/apps/vaultwarden/`. Migración: contenedor parado, `data/`+compose+`.env` empaquetados con checksum SHA-256 verificado en origen/Mac/destino, restaurados en `services`. Puerto publicado `8082` en todas las interfaces (antes `127.0.0.1:8082`: ahora necesita ser alcanzable desde otro host, el túnel de Cloudflare en `core01`). Ingress rule de Cloudflare repuntado a `192.168.0.154:8082`; Homepage ya usaba el dominio público, sin cambios. Ver [Vaultwarden](../servicios/vaultwarden.md). |
| DocuSeal (imagen `latest`) | Docker en `services` | Actual, instalado 2026-09-23, sin cuenta admin todavía | `/srv/oscar/apps/docuseal/`, standalone (SQLite + storage local embebidos en la imagen oficial, sin Postgres separado — alcanza para el volumen de uso de un homelab). `SECRET_KEY_BASE` generado único con `openssl rand`. Acceso `docuseal.oscar.home` vía NPM. **Falta terminar el setup inicial** (crear la cuenta admin) — paso manual por la web, no se hizo por el usuario todavía. |
...
| `automation` — Ubuntu 24.04 LTS + Docker 29 | VM (vmid 107) | Actual, creada 2026-09-23 | 2 vCPU / 4 GB / 60 GB disco, mismo patrón de clon que las demás. IP estática `192.168.0.153/24`, DNS `192.168.0.213` (AdGuard de `network`) + `1.1.1.1`. Paso 4 de la reorganización del rack — ver `REORGANIZACION_RACK.md`. Primer servicio: n8n+Postgres, migrado desde `core01`. **Gotcha real:** el clon del template quedó con el disco original de 3.5GB (el `qm resize` no está en la receta de `crear-vm-core01.md`, se hizo a mano después) — `docker install` falló por falta de espacio hasta corregirlo (`qm resize` + `growpart` + `resize2fs` en caliente, sin reboot). |
| `services` — Debian 13 + Docker 29 | **LXC unprivileged** (vmid 108) | Actual, creada 2026-09-23 | 2 vCPU / 2 GB / 16 GB disco, template `debian-13-standard`, `features: nesting=1,keyctl=1` (necesario para que Docker corra dentro de un LXC sin privilegios — probado con `docker run hello-world` real, no asumido). IP estática `192.168.0.154/24`. Paso 5 de la reorganización del rack. Servicios: Vaultwarden (migrado de `core01`, expuesto vía Cloudflare Tunnel en `vault.oscarlab.com.ar`), SearXNG y DocuSeal (SearXNG migrado de k3s, DocuSeal nuevo — los dos vía NPM, `searxng`/`docuseal`.oscar.home, solo LAN). |
| Minecraft (Java, Paper + Geyser/Floodgate) | Docker en `core01` | Actual | `itzg/minecraft-server:java21`, `/srv/oscar/apps/minecraft/`, puerto `25565` (Java) + `19132/udp` (Bedrock — PS5/Switch/celular vía Geyser+Floodgate, 2026-09-20). Sin whitelist todavía — cualquiera con la IP:puerto entra. Ver [Minecraft](../juegos/minecraft.md). |
...
| `oscar-compose` (repo nuevo) | Forgejo en `devops01` | Actual | `compose.yaml` de Minecraft/CS2/Infisical versionados. Los secrets ya no se mantienen a mano — `deploy.sh` por app hace `infisical export` (regenera `.env` desde Infisical) antes de `docker compose up -d`. |
| Infisical Secrets Operator | k3s en `k3s01` (namespace `infisical-operator-system`) | Actual | Sincroniza `Secret`s nativos de k8s desde Infisical vía el CRD `InfisicalSecret` — desplegado por Argo CD (`infra/infisical-operator/`, chart oficial). Migró el `imagePullSecret` real de `ci-demo`. El `clientId`/`clientSecret` que autentica al operador vive en un Secret creado a mano, nunca en Git — mismo criterio que `nexus-pull`. Ver [gestión de secretos](../seguridad/secretos.md#secrets-en-gitops-k3s--argo-cd). |
