...
| Forgejo 16.0.4 | Docker en `devops01` | Actual | `/srv/oscar/apps/forgejo/`, `git.oscar.home` (rewrite en AdGuard) HTTP `3000` / SSH `2222`→22 interno, SQLite, auto-registro deshabilitado desde el arranque. Admin creado, credencial en Vaultwarden. Falta sumarlo a Uptime Kuma y el CI Runner. Ver [Forgejo / Git local](../servicios/forgejo.md). |
