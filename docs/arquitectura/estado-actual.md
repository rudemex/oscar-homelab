...
| Tailscale | `pinode01` (subnet router principal) y `core01` (respaldo) | Actual | `192.168.0.0/24` advertido y aprobado en los dos (2026-09-21) — el principal es `pinode01`, así el acceso remoto ya no depende de que el Dell esté arriba. Cualquier dispositivo del tailnet alcanza la LAN de casa por IP; los hostnames `*.oscar.home` no resuelven (ver limitación en [acceso remoto](../red/acceso-remoto.md)). Ver [ADR-011](./decisiones-arquitectonicas.md#adr-011--tailscale-como-vpn-de-acceso-remoto) y [acceso remoto](../red/acceso-remoto.md). |
