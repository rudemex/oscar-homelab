...
**Qué se intentó** (2026-09-21), todo sin éxito: rescan del dispositivo SCSI; detener la VM 101, soltar el montaje, quitar `sda` del kernel y reescanear el bus (el puerto `ata1` quedó en `hardreset failed`); y finalmente **apagar el host, cortar la energía y reasentar el cable SATA**. Tras ese arranque en frío el puerto sigue igual: `ata1: link is slow to respond (ready=0)` → `limiting SATA link speed to 3.0 Gbps` → `hardreset failed` → `reset failed, giving up`. También se **reinició la controladora SATA completa** (`00:17.0`, remove + rescan de PCI) con el mismo resultado. Es decir, hay enlace físico pero el dispositivo nunca queda listo, lo típico de un **SSD con el controlador muerto** (o, menos probable, cable/puerto). `lsblk` solo ve el NVMe; el storage `Backups` quedó **deshabilitado** en Proxmox (`pvesm set Backups --disable 0` cuando el disco vuelva).

**Conclusión:** no es recuperable por software y **no se puede formatear un disco que el sistema no ve**. La causa más probable es un SSD (`FTM1TN325H`) con el firmware colgado o el controlador muerto; el siguiente paso es probarlo fuera del Dell. Formatear no lo arreglaría y destruiría los `vzdump` que pudieran leerse.
