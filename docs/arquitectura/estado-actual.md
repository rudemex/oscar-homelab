...
| Forgejo 16.0.4 | Docker en `devops01` | Actual | `/srv/oscar/apps/forgejo/`, `http://git.oscar.home` (sin puerto — nginx propio en `:80` proxea a `:3000`, ver [Forgejo / Git local](../servicios/forgejo.md#reverse-proxy-nginx)) / SSH `2222`→22 interno, SQLite, auto-registro deshabilitado desde el arranque. Admin creado, credencial en Vaultwarden, monitor en Uptime Kuma. Falta el CI Runner. Dependencia real: solo resuelve en dispositivos con DNS apuntado a mano a AdGuard (`192.168.0.93`) — no es DNS de toda la LAN hoy. |
| nginx 1.27 | Docker en `devops01` | Actual | `/srv/oscar/apps/nginx/`, `network_mode: host`, reverse proxy por hostname en el puerto 80 — hoy solo `git.oscar.home`, preparado para sumar `nexus.oscar.home` cuando llegue. |
