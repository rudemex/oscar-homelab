...
| Infisical Secrets Operator | k3s en `k3s01` (namespace `infisical-operator-system`) | Actual | Sincroniza `Secret`s nativos de k8s desde Infisical vía el CRD `InfisicalSecret` — desplegado por Argo CD (`apps/infisical-operator/`, chart oficial). Validado de punta a punta con un ejemplo real (`apps/infisical-secrets/`, namespace `oscar-lab`). El `clientId`/`clientSecret` que autentica al operador vive en un Secret creado a mano, nunca en Git — mismo criterio que `nexus-pull`. Ver [gestión de secretos](../seguridad/secretos.md#secrets-en-gitops-k3s--argo-cd). |
