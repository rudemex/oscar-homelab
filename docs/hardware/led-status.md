...
Todo lo anterior refleja **un único estado global** (el de O.S.C.A.R. mismo). `rackStatus` es un efecto manual distinto (`POST /effects {"effect":"rackStatus"}`, o el botón **Rack** en `/panel`) que pinta **una fila `MirrorZone` por host real del rack** — Proxmox, las 8 VMs/LXC del Dell y las 2 Raspberry Pi — verde si el puerto TCP responde, rojo si no, repintando cada 15 s por defecto (`pollIntervalMs` configurable en el body).

El mapeo fila→host vive en `apps/oscar-led-controller/src/core/rackStatus/rackTargets.ts` (filas 14-24, checkeo TCP puro — puerto 22 en las VMs/LXC/Pi, 8006 en Proxmox — sin exporters ni Prometheus de por medio). El orden es de **dependencia**, no de posición física exacta: casi todo (Proxmox + 8 VMs/LXC) vive en el mismo Dell OptiPlex de U7 del rack, así que alinear por altura real no aportaría información — ver [`RackTarget`](https://github.com/rudemex/oscar-homelab) en el código para el detalle completo.
