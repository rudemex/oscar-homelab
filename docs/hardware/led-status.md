...
**Quinta integración real, mismo día: `booting` vía un servicio de systemd en el host Proxmox.** El controlador vive en k3s, que arranca *después* del Dell — no puede avisar de su propio arranque desde adentro. Por eso el disparador corre en el host: [`oscar-led-boot.service`](../proxmox/operacion.md#servicios-y-scripts-propios-del-host-2026-09-25) espera a que la API de la tira responda tras un boot, reproduce `booting` (~35 s) y, solo si nadie tomó la tira mientras tanto, la asienta en `healthy`. Como la API solo responde cuando k3s ya levantó, la animación se lee como "OSCAR terminó de despertar", no como "el Dell acaba de encender". **Observado en un reinicio real del Dell (2026-09-25, 21:12):** la tira quedó en `maintenance` (declarado con `oscar-maintenance start` antes de reiniciar) mientras el host estuvo abajo, la API volvió a las 21:15:56 y `booting` arrancó 4 s después. El camino de espera con la API caída funcionó. El reinicio destapó dos defectos, ya corregidos: ver [Argo CD](../servicios/argocd.md#hallazgo-del-primer-reinicio-real-2026-09-25-notificaciones-repetidas) y el ajuste de los bridges (`booting` también los frena, igual que `maintenance`).

**Sexta, mismo día: `night` por horario.** Dos timers de systemd en el host Proxmox: a las 23:00 `healthy` → `night` y a las 07:00 `night` → `healthy` (hora `America/Argentina/Buenos_Aires`). Solo intercambian entre esos dos estados, así que nunca pisan `critical`, `backup`, `maintenance` ni un modo elegido a mano. Los horarios se cambian editando `OnCalendar=` en los timers. Detalle en [operación de Proxmox](../proxmox/operacion.md#servicios-y-scripts-propios-del-host-2026-09-25).
