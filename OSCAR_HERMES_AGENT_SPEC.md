...
- [x] **Test 2 — renovación de token: aprobado** (2026-09-21). El access token venció `2026-09-21T01:34:17Z`; ~11 h después (12:16Z) el Pod seguía con `loggedIn: true` (`claude auth status`, sin re-login) y `claude -p "responde solo: ok"` respondió `ok`. El `.credentials.json` quedó con `expiresAt` = `2026-09-21T20:16:22Z` (+8 h desde ese uso): el refresh token renovó solo, con el access token ya vencido. Matiz: la renovación ocurre **al usarse**, no de forma proactiva — un token sin uso no se renueva hasta el próximo `claude`. Pendiente separado: repetir la ronda simultánea Mac+Pod tras esta primera renovación y el Test 4 (72 h, hasta `2026-09-23T17:34Z`).
